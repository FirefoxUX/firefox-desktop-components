...
module.exports = __webpack_require__.p + "placeholder-message.a8810b657e1cc88d807c.css";
  ...
  _checkForControllingExtension = async () => {
    ...
    if (info && info.id) {
      ...
      if (addon) {
        ...
        this.controllingExtensionInfo.mayDisable = !!(addon.permissions & lazy.AddonManager.PERM_CAN_DISABLE);
        this.emit("change");
        return;
      }
    }
...
var SyncHelpers = new class SyncHelpers {
  ...
  maybeShowSyncAction() {
    if (location.hash == "#sync" && window.UIState.get().status == window.UIState.STATUS_SIGNED_IN) {
      if (location.href.includes("action=pair")) {
        window.gSubDialog.open("chrome://browser/content/preferences/fxaPairDevice.xhtml", {
...
document.addEventListener("paneshown", (/** @type {PaneShownEvent} */event) => {
...
//# sourceMappingURL=sync-engine-list-sync-engines-list-stories.6da7ce7d.iframe.bundle.js.map