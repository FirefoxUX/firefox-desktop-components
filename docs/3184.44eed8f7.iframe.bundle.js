"use strict";
(self["webpackChunk"] = self["webpackChunk"] || []).push([[3184],{
...
ChromeUtils.defineESModuleGetters(lazy, {
  ...
  PopupAndRedirectBlocker: "moz-src:///toolkit/actors/PopupAndRedirectBlockingParent.sys.mjs",
...
ChromeUtils.defineLazyGetter(lazy, "blankURI", () => Services.io.newURI("about:blank"));
let lazyPrefs = {};
XPCOMUtils.defineLazyPreferenceGetter(lazyPrefs, "unloadTimeoutMs", "dom.beforeunload_timeout_ms");
Object.defineProperty(lazy, "ProcessHangMonitor", {
  ...
  get() {
    ...
    if (Cu.isESModuleLoaded(kURL)) {
      let {
        ProcessHangMonitor
      } = ChromeUtils.importESModule(kURL);
...
class MozBrowser extends MozElements.MozElementMixin(XULFrameElement) {
  ...
  constructor() {
    ...
    this.addEventListener("drop", event => {
      const contentAnalysis = Cc["@mozilla.org/contentanalysis;1"].getService(Ci.nsIContentAnalysis);
      if (contentAnalysis.isActive) {
        ...
        try {
          ...
          let request = lazy.ContentAnalysisUtils.createContentAnalysisRequest({
            analysisType: Ci.nsIContentAnalysisRequest.eBulkDataEntry,
            operationTypeForDisplay: Ci.nsIContentAnalysisRequest.eDroppedText,
          ...
          contentAnalysis.analyzeBatchContentRequest(request, true).then(caResult => {
            let shouldAllowContent = true;
            if (caResult.length > 1) {
              shouldAllowContent = caResult[1].shouldAllowContent;
            }
            dragSession.sendDispatchToDropTargetAndResumeEndDragSession(shouldAllowContent, caResult[0]);
          }, () => {
            dragSession.sendDispatchToDropTargetAndResumeEndDragSession(false, []);
          });
  ...
  get browsingContext() {
    if (this.frameLoader) {
      return /** @type {CanonicalBrowsingContext} */this.frameLoader.browsingContext;
    }
    return null;
  }
  /**
  ...
  get contentTitle() {
    return (this.isRemoteBrowser ? this.browsingContext?.currentWindowGlobal?.documentTitle : this.contentDocument.title) ?? "";
  }
  ...
  get contentPartitionedPrincipal() {
    return this.isRemoteBrowser ? this._contentPartitionedPrincipal : this.contentDocument.partitionedPrincipal;
  }
  ...
  goBack(requireUserInteraction = lazy.BrowserUtils.navigationRequireUserInteraction) {
    var webNavigation = this.webNavigation;
    if (requireUserInteraction ? webNavigation.canGoBack : webNavigation.canGoBackIgnoringUserInteraction) {
      this._wrapURIChangeCall(() => webNavigation.goBack(requireUserInteraction));
    }
  }
  goForward(requireUserInteraction = lazy.BrowserUtils.navigationRequireUserInteraction) {
    var webNavigation = this.webNavigation;
  ...
  onPageHide() {
    ...
    var tabBrowser = this.getTabBrowser();
    if (!tabBrowser || !("fastFind" in tabBrowser) || tabBrowser.selectedBrowser == this) {
  ...
  unselectedTabHover(hovered) {
    ...
    this.sendMessageToActor("Browser:UnselectedTabHover", {
      hovered
    }, "UnselectedTabHover", "roots");
  }
  didStartLoadSinceLastUserTyping() {
    return !this.isNavigating && this.urlbarChangeTracker._startedLoadSinceLastUserTyping;
  }
  constrainPopup(popup) {
    if (this.getAttribute("constrainpopups") != "false") {
      let constraintRect = this.getBoundingClientRect();
      constraintRect = new DOMRect(constraintRect.left + window.mozInnerScreenX, constraintRect.top + window.mozInnerScreenY, constraintRect.width, constraintRect.height);
  ...
  purgeSessionHistory() {
    try {
      ...
      if (this.browsingContext.currentWindowGlobal.documentURI != "about:blank") {
  ...
  createAboutBlankDocumentViewer(aPrincipal, aPartitionedPrincipal) {
    let principal = lazy.BrowserUtils.principalWithMatchingOA(aPrincipal, this.contentPrincipal);
    let partitionedPrincipal = lazy.BrowserUtils.principalWithMatchingOA(aPartitionedPrincipal, this.contentPartitionedPrincipal);
    if (this.isRemoteBrowser) {
      this.frameLoader.remoteTab.createAboutBlankDocumentViewer(principal, partitionedPrincipal);
    } else {
      this.docShell.createAboutBlankDocumentViewer(principal, partitionedPrincipal);
    }
  }
  _acquireAutoScrollWakeLock() {
    const pm = Cc["@mozilla.org/power/powermanagerservice;1"].getService(Ci.nsIPowerManagerService);
  ...
  stopScroll() {
    if (this._autoScrollBrowsingContext) {
      ...
      if (autoScrollWnd) {
        autoScrollWnd.getActor("AutoScroll").sendAsyncMessage("Autoscroll:Stop", {});
      }
      try {
      ...
      if (this._autoScrollScrollId != null) {
        this._autoScrollBrowsingContext.stopApzAutoscroll(this._autoScrollScrollId, this._autoScrollPresShellId);
        this._autoScrollScrollId = null;
        this._autoScrollPresShellId = null;
      }
      this._autoScrollBrowsingContext = null;
  ...
  startScroll({
    ...
    let screen = screenManager.screenForRect(screenXDesktopPx, screenYDesktopPx, 1, 1);
    ...
    if (screen.colorDepth > 8) {
      ...
      this._autoScrollPopup.setAttribute("translucent", AppConstants.platform == "win" || AppConstants.platform == "macosx");
    }
    ...
    let popupY;
    {
      let cssToDesktopScale = window.devicePixelRatio / window.desktopToDeviceScale;
      ...
      popupX = Math.max(minX, Math.min(maxX, screenXDesktopPx)) / cssToDesktopScale;
      popupY = Math.max(minY, Math.min(maxY, screenYDesktopPx)) / cssToDesktopScale;
    }
    ...
    let usingApz = false;
    if (scrollId != null && this.mPrefs.getBoolPref("apz.autoscroll.enabled", false)) {
      ...
      usingApz = browsingContext.startApzAutoscroll(screenXDevPx, screenYDevPx, scrollId, presShellId);
        ...
        case "DOMMouseScroll":
          {
            ...
            const scrollCooldownMs = this.mPrefs.getIntPref("apz.autoscroll.scroll_wheel_cooldown");
            if (performance.now() - this._autoScrollStartTime < scrollCooldownMs) {
  ...
  swapDocShells(aOtherBrowser) {
    ...
    if (this.isRemoteBrowser) {
      fieldsToSwap.push(...["_remoteWebNavigation", "_remoteFinder", "_documentURI", "_documentContentType", "_characterSet", "_mayEnableCharacterEncodingMenu", "_contentPrincipal", "_contentPartitionedPrincipal", "_isSyntheticDocument", "_originalURI", "_userTypedValue"]);
    }
    var ourFieldValues = {};
  ...
  getInPermitUnload(aCallback) {
    if (this.isRemoteBrowser) {
      ...
      aCallback(this._inPermitUnload.has(this.browsingContext.currentWindowGlobal));
      return;
    }
  ...
  async asyncPermitUnload(action) {
    ...
    try {
      let permitUnload = await wgp.permitUnload(action, lazyPrefs.unloadTimeoutMs);
      return {
        permitUnload
      };
    } finally {
  ...
  permitUnload(action) {
    if (this.isRemoteBrowser) {
      ...
      if (lazy.ProcessHangMonitor?.findActiveReport(this) || lazy.ProcessHangMonitor?.findPausedReport(this)) {
        return {
          permitUnload: true
        };
      }
      let result;
      let success;
      this.asyncPermitUnload(action).then(val => {
        result = val;
        success = true;
      }, err => {
        result = err;
      ...
      Services.tm.spinEventLoopUntilOrQuit("browser-custom-element.js:permitUnload", () => window.closed || success !== undefined);
      if (success) {
        return result;
      }
      throw result;
  ...
  async drawSnapshot(x, y, w, h, scale, backgroundColor, fullViewport = false) {
    ...
    try {
      return this.browsingContext.currentWindowGlobal.drawSnapshot(rect, scale, backgroundColor);
    } catch (e) {
      return false;
    }
  }
  ...
  sendMessageToActor(messageName, args, actorName, scope) {
    ...
    function sendToChildren(browsingContext, childScope) {
      ...
      if (windowGlobal && (childScope != "roots" || windowGlobal.isProcessRoot)) {
  ...
  leaveModalState() {
    this.sendMessageToActor("LeaveModalState", {
      forceLeave: true
    }, "BrowserElement", "roots");
  }
...
//# sourceMappingURL=3184.44eed8f7.iframe.bundle.js.map